...
def get_metrics(flight_names, model, data):

    results = pd.DataFrame(columns=['flight_name', 'accuracy', 'recall', 'fpr', 'f1'])

    df = data[['altitude', 'latitude', 'longitude', 'groundspeed']]
    y_pred = pd.DataFrame(model.predict(df))

    test_score_df = pd.DataFrame()
    test_score_df['label'] = data['label']

    y_pred.replace(1, 0, inplace=True)
    y_pred.replace(-1, 1, inplace=True)

    test_score_df['anomaly'] = y_pred.values
    ...
    res = pd.Series([flight_names, acc, recall, fpr, f1],  index=results.columns)
    ...
    for scenario_name, scenario_path in scenarios.items():
        ...
        for folder_path in glob.glob(f'{scenario_path}/*'):
            ...
            for file in glob.glob(f"{folder_path}/*.pkl"):
                ...
                data = normalize(data_raw, dep_airp, arr_airp)
                temp_results = get_metrics(flight_names, model, data)
